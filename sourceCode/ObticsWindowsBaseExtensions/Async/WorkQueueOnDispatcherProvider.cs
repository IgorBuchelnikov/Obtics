...
{
    /// <summary>
    /// Default <see cref="IWorkQueueProvider"/> based on <see cref="Dispatcher"/>.
    /// Schedules workitems with priority <see cref="DispatcherPriority.DataBind"/>.
    /// </summary>
    public class WorkQueueOnDispatcherProvider : IWorkQueueProvider, IPrioritizedWorkQueueProvider
    {
        [ThreadStatic]
                ...
                if (_Adapter == null)                
                {
                    _Adapter =
                        HighLowPrioritizedWorkQueueAdapter.Get(
                            WorkQueueOnDispatcherAdapter.Get(Dispatcher.CurrentDispatcher, DispatcherPriority.DataBind)
                        );
                }
