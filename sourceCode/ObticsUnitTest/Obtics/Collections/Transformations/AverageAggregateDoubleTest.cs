...
namespace ObticsUnitTest.Obtics.Collections.Transformations
{
    ...
    /// </summary>
    [TestClass]
    public class AverageAggregateDoubleTest
    {
        public AverageAggregateDoubleTest()
        {
            //
        ...
        public void ConcurrencyTest()
        {
            List<double> source = new List<double>(
               new double[] { 1d, 2d, 3d, 4d, 5d, 6d, 7d, 8d, 9d, 10d, 11d, 12d, 13d }
           );

            AsyncTestRunnerForCollectionAggregate.Run(
                source.ToArray(),
                5.5d,
                frame => AverageAggregateDouble.Create(frame).OnException((InvalidOperationException ex) => 222f),
                "AverageAggregateDouble.Create(frame)"
            );
        }
        ...
        class CorrectnessRunner : CollectionTestSequenceRunnerForAggregate<double, double>
        {
            ...
            {
                return AverageAggregateDouble.Create(frame).OnException((InvalidOperationException ex) => 222.0);
            }

            public override string Prefix
            {
                get { return "AverageAggregateDouble.Create(frame)"; }
            }
        }
        ...
        {
            Assert.IsNull(AverageAggregateDouble.Create(null), "Should return null when source is null.");
        }
        ...
        public void EqualityTest()
        {
            EqualityRunner.Run(
                ...
                s => AverageAggregateDouble.Create(s),
                "AverageAggregateDouble"
            );
        }
    }
}
