...
namespace ObticsUnitTest.Obtics.Collections.Transformations
{
    ...
    /// </summary>
    [TestClass]
    public class AverageAggregateDecimalTest
    {
        public AverageAggregateDecimalTest()
        {
            //
        ...
        public void ConcurrencyTest()
        {
           List<decimal> source = new List<decimal>(
            ...
            AsyncTestRunnerForCollectionAggregate.Run(
                source.ToArray(),
                5.5m,
                frame => AverageAggregateDecimal.Create(frame).OnException( (InvalidOperationException ex) => 222m ),
                "AverageAggregateDecimal.Create(frame)"
            );
        }
        ...
        class CorrectnessRunner : CollectionTestSequenceRunnerForAggregate<decimal, decimal>
        {
            ...
            static List<decimal> filler = new List<decimal>(
                new decimal[] { -4m, 5.3m, -3m, -18.45m, 14m, 12.223m }
            );
            ...
            {
                return
                    ValueProvider.OnException<decimal,InvalidOperationException>(
                        (IValueProvider<decimal>)AverageAggregateDecimal.Create(frame),
                        (InvalidOperationException ex) => 222m
                    );
            }

            public override string Prefix
            {
                get { return "AverageAggregateDecimal.Create(frame)"; }
            }
        }
        ...
        class CorrectnessRunner2 : CollectionTestSequenceRunnerForAggregate<decimal, decimal>
        {
            ...
            public override IValueProvider<decimal> CreatePipeline(FrameIEnumerable<decimal> frame)
            {
                return AverageAggregateDecimal.Create(frame).OnException((InvalidOperationException ex) => 222m);
            }
        ...
        {
            Assert.IsNull(AverageAggregateDecimal.Create(null), "Should return null when source is null.");
        }
        ...
        public void EqualityTest()
        {
            EqualityRunner.Run(
                ...
                s => AverageAggregateDecimal.Create(s),
                "AverageAggregateDecimal"
            );
        }
    }
}
